...
bool ProcessManager::WaitForProcessExit(const char* ProcessName)
{
	PROCESSENTRY32 Process;

	if (!FindProcessByNameA(ProcessName, &Process))
		return true;

	return WaitForProcessExit(Process.th32ProcessID);
}

bool ProcessManager::WaitForProcessExit(unsigned long ProcessID)
{
	HANDLE ProcessHandle;
