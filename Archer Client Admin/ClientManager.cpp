...
void ClientManager::Attach()
{
	Instance = this;
}

bool ClientManager::Initialize(WSAPROTOCOL_INFOA* ProtocolInfo)
{
	...
	if (ProtocolInfo)
	{
		...
		{
			MessageBoxA(0, __CS("Failed to connect"), "Failure", MB_ICONERROR);
			return false;
		}

		Instance->ClientSocket = WSASocketA(0, 0, 0, ProtocolInfo, 0, WSA_FLAG_OVERLAPPED);
	...
	{
		Instance->ClientSocket = Instance->Connect();
		if (Instance->ClientSocket == INVALID_SOCKET)
			return false;
	}
...
unsigned long long ClientManager::GetDataHash(const void* Data, unsigned long Size)
{
...
unsigned long long ClientManager::GetDataHashOffset(const void* Data, unsigned long Size)
{
...
bool ClientManager::VerifyHash(const void* Data, unsigned long Size, unsigned long long SubHash)
{
	unsigned long long Hash;
...
bool ClientManager::Send(ArcherType Type, const void* Buffer, unsigned long Size)
{
