...
#define RESULT_VALID(SocketResult) ((SocketResult) && ((SocketResult) != ~0))
#define RESULT_INVALID(SocketResult) (!(SocketResult) || ((SocketResult) == ((unsigned short)~0)))
...
#define MAX_IP_LENGTH ((3 * 4) + 3 + 1)

class ClientManager
{
public:
	inline ClientManager()
	{
	}

	ClientManager(bool AllowSizeDifference);

public:
	static void StartListening();

private:
	static void TerminateHandler(unsigned int IPAddress);
	static unsigned int GetSocketIPAddress(unsigned int ClientSocket);

public:
	constexpr unsigned int GetIPAddress()
	{
		return IPAddress;
	}
...
public:
	...
	void GetIPAddress(char* Buffer);

	bool Send(ArcherType Type, void* Buffer, unsigned int Size);
	ArcherType Receive(void* Buffer, unsigned int Size, unsigned int* ReceiveSize, unsigned long long Timeout = 0);
...
private:
	...
	void DecryptPacket(unsigned long long Key, void* Data, unsigned int Size);

	bool GetNearestDescriptor(ArcherPacket* Descriptor, unsigned long long Timeout = 0, bool Progress = false);
	bool VerifyHash(void* Data, unsigned int Size, unsigned long long SubHash);

	unsigned long long GetDataHash(void* Data, unsigned int Size);
	unsigned long long GetDataHashOffset(void* Data, unsigned int Size);

	ArcherType ReadIntoBuffer(void* Buffer, unsigned int Size, unsigned long long Timeout = 0);
	bool ReceiveData(void* Buffer, unsigned int Size, unsigned long long Timeout = 0, bool Progress = true);

private:
	static volatile unsigned int WaitingConnection;

	static Mutex HandlerLock;
	static HashMap<unsigned int, ClientManager*> IPHandlers;

private:
	bool InfractSizeDifference;
